import { position } from "../engine/weeklyManager.js";
...
export function weeklyPageContext(url, kind, week) {
  ...
  const route = /^\/f1\/(\d+)\/(players|\d+)\/?$/.exec(source.pathname);
  ...
  const stat = source.searchParams.get("stat1");
  ...
  if (kind === "waivers" && !projectionWeek) throw new Error("On Yahoo Players, select this week's projected stats before importing (not season totals or actual points).");
...
export function parseWeeklyPage(doc, { url, kind, season, week, scoring, now = Date.now() }) {
  const context = weeklyPageContext(url, kind, week);
  ...
  for (const table of doc.querySelectorAll("table")) {
    ...
    const statusCol = cols.findIndex((h) => /^(status|availability)(?:\s+\1)?$/.test(h));
    ...
    for (const tr of table.querySelectorAll("tbody tr")) {
      ...
      const add = [...tr.querySelectorAll("a[href]")].find((a) => /\/(addplayer|waiver)(?:[/?]|$)/.test(a.getAttribute("href")));
      const availability = /^FA\b/.test(availabilityText) ? "FA" : /^W(?:\b|\()/.test(availabilityText) ? "W" : add ? (/waiver|claim/i.test(`${add.getAttribute("href")} ${add.getAttribute("title")} ${clean(add)}`) ? "W" : "FA") : null;
      ...
      players.push({ name, team: teamPos[1].toUpperCase(), pos: position(teamPos[2].split(",")[0]),
...
export function mergeWaiverPages(previous, incoming) {
  ...
  for (const page of all) for (const p of page.players) byName.set(p.name.toLowerCase(), p);
  return { ...incoming, capturedAt: all[0].capturedAt, players: [...byName.values()], pages };
}
